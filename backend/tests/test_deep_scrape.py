...
from deep_scrape import scrapeticker, PDF_FOLDER

@pytest.fixture(autouse=True)
def mock_external_dependencies():
    with (
        patch('deep_scrape.openai.ChatCompletion.create') as mock_openai_create,
        patch('deep_scrape.sync_playwright') as mock_sync_playwright,
        patch('deep_scrape.requests.get') as mock_requests_get,
        patch('os.path.exists') as mock_os_path_exists,
        patch('builtins.open', new_callable=MagicMock) as mock_open,
        patch('os.remove') as mock_os_remove,
        patch('deep_scrape.os.makedirs') as mock_makedirs,
    ): 
